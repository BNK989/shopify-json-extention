      ...
      try {
        ...
        if (typeof window.getMetafieldData === 'function') {
          ...
          if (metafieldData && !metafieldData.error) {
            ...
            console.warn('Metafield data error:', metafieldData.error);
          }
        } else {
          console.warn('getMetafieldData function is not yet available');
        }
      } catch (error) {
...
// Function to insert data into a container
function insertDataToDiv(data, container) {
  if (!container) {
...
let hasInitialized = false; // Add this flag to track initialization
